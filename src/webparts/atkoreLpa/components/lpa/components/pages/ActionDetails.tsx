...
export function ActionDetails({ actionId, onNavigate, onBack, onExit }: ActionDetailsProps) {
  ...
  const displayAction = action || fallbackAction;

  const handleCaptureEvidence = () => {
    const fileName = `action-${actionId}-${Date.now()}.jpg`;
    const blob = new Blob(['closure-evidence'], { type: 'image/jpeg' });
    service
      .uploadActionClosureEvidence(fileName, blob, {
        ActionReference: actionId
      })
      .then((url) => {
        setEvidence([...evidence, url || fileName]);
      })
      .catch(() => {
        setEvidence([...evidence, fileName]);
      });
  };

  const handleCloseAction = () => {
    setSubmitting(true);
    const itemId = displayAction.itemId || Number(String(actionId).replace(/[^\d]/g, ''));
    Promise.resolve()
      .then(() => {
        if (itemId) {
          return service.updateActionStatus(itemId, closureStatus, closureNotes);
        }
        return undefined;
      })
      .finally(() => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F5F8F5]">
        <AppHeader title="Action Details" onExit={onExit} actions={
          <button onClick={onBack} className="p-2 hover:bg-gray-100 rounded">
            <ChevronLeft size={20} />
          </button>
        } />
        <div className="flex items-center justify-center h-64">
          <p className="text-text-secondary">Loading action details...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-4 md:p-6 space-y-6 pb-24 max-w-7xl mx-auto">
        ...
        <Card>
          <div className="flex items-start justify-between gap-3 mb-5">
            <h3>{displayAction.id}</h3>
            {getStatusBadge(displayAction.status)}
          </div>

          <div className="space-y-5">
            <div>
              <p className="text-label mb-2">Description</p>
              <p className="text-value-large">{displayAction.description}</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-5 pt-5 border-t border-[rgba(0,0,0,0.08)] sm:grid-cols-2">
              <div>
                ...
                <p className="text-value">{displayAction.department}</p>
              </div>
              <div>
                <p className="text-label mb-2">Action Due Date</p>
                <p className="text-value text-[#D13438]">
                  {new Date(displayAction.dueDate).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })}
                </p>
              </div>
              <div>
                <p className="text-label mb-2">Auditor Name</p>
                <p className="text-value">{displayAction.auditorName}</p>
              </div>
              <div>
                <p className="text-label mb-2">Category</p>
                <p className="text-value">{displayAction.category}</p>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="space-y-5">
            <p className="text-caption text-[#9CA3AF]">
              Audit Date: {new Date(displayAction.auditDate).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })}
            </p>

            <div>
              <p className="text-label mb-2">Question</p>
              <p className="text-body-medium">{displayAction.originalQuestion}</p>
            </div>

            <div>
              <p className="text-label mb-2">Auditor Notes</p>
              <div className="mt-2 p-4 bg-[rgba(0,0,0,0.03)] rounded-lg border-l-2 border-[#6B7280]">
                <p className="text-body-medium italic">{displayAction.auditorNotes}</p>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          {displayAction.status === 'closed' && displayAction.closureDetails ? (
            ...
            <div className="bg-[#E8F5E9] p-4 rounded-lg border border-[#4CAF50]/20 space-y-5">
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
                <div>
                  ...
                  <p className="text-value">{displayAction.closureDetails.closedBy}</p>
                </div>
                <div>
                  <p className="text-label mb-2">Closed Date</p>
                  <p className="text-value">
                    {new Date(displayAction.closureDetails.closedDate).toLocaleDateString('en-US', { month: '2-digit', day: '2-digit', year: 'numeric' })}
                  </p>
                </div>
              </div>
              <div>
                ...
                <div className="p-4 bg-white rounded-lg border border-[#4CAF50]/10">
                  <p className="text-body-medium">{displayAction.closureDetails.workOrderNo}</p>
                </div>
              </div>
              <div>
                <p className="text-label mb-2">Closure Notes</p>
                <div className="p-4 bg-white rounded-lg border border-[#4CAF50]/10">
                  <p className="text-body-medium">{displayAction.closureDetails.closureNotes}</p>
                </div>
              </div>
              {displayAction.closureDetails.closureEvidence && displayAction.closureDetails.closureEvidence.length > 0 && (
                <div>
                  <p className="text-label mb-3">Closure Evidence</p>
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {displayAction.closureDetails.closureEvidence.map((evidence: string, index: number) => (
