...
export function AuditorHome({ onNavigate, onExit }: AuditorHomeProps) {
  ...
  const normalizeAuditStatus = (status?: string): Audit['status'] => {
    const normalized = String(status || '')
      .trim()
      .toLowerCase()
      .replace(/[\s_-]+/g, '');

    if (normalized === 'pendingacceptance' || normalized === 'pending') {
      return 'pending';
    }
    ...
    const loadData = async () => {
      try {
        const [audits, actions] = await Promise.all([
          service.getAuditsForRole('auditor'),
          service.getActionsForRole('auditor')
        ]);
        ...
        setLiveActions(
          actions
            ...
            .map((item) => ({
              id: item.Title,
              itemId: item.Id,
              auditId: item.AuditReference || 'AUD',
              department: item.DepartmentName || 'Department',
  ...
  const handleUpdateStatusSubmit = () => {
    if (selectedAction?.itemId) {
      service.updateActionStatus(selectedAction.itemId, newStatus, updateNote).catch(() => undefined);
      setLiveActions((prev) =>
        prev.map((item) =>
          item.itemId === selectedAction.itemId ? { ...item, status: newStatus } : item
        )
      );
    }
  ...
  const handleAcceptAudit = () => {
    if (selectedAudit?.itemId) {
      service.acknowledgeAudit(selectedAudit.itemId, true).catch(() => undefined);
      setLiveAudits((prev) =>
        prev.map((item) =>
          item.itemId === selectedAudit.itemId ? { ...item, status: 'accepted' } : item
        )
      );
    }
  ...
  const handleDenyAudit = () => {
    ...
    if (selectedAudit?.itemId) {
      service.acknowledgeAudit(selectedAudit.itemId, false, denyReason).catch(() => undefined);
      setLiveAudits((prev) =>
        prev.map((item) =>
          item.itemId === selectedAudit.itemId ? { ...item, status: 'denied' } : item
        )
      );
    }
  ...
  const auditSource: Audit[] = liveAudits;
  const actionSource: Action[] = liveActions;
  ...
  const todayAudits = auditSource.filter((a) => {
    const due = toDateOnly(a.dueDate);
    return due.getTime() <= todayDate.getTime() && a.status !== 'submitted';
  });
  const upcomingAudits = auditSource.filter((a) => {
    const due = toDateOnly(a.dueDate);
    return due.getTime() > todayDate.getTime() && a.status !== 'submitted';
  });
  const completedAudits = auditSource.filter(a => a.status === 'submitted');
  const auditsByView: Record<AuditView, Audit[]> = {
    today: todayAudits,
    upcoming: upcomingAudits,
    completed: completedAudits
  };
  const selectedAudits = auditsByView[activeTab];

  // Get unique audit IDs and departments for filters
  const uniqueAudits = Array.from(new Set(actionSource.map(a => a.auditId)));
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="space-y-5 md:px-[100px] py-6">
        ...
        <div className="hidden md:flex md:items-center md:justify-between md:gap-4 px-4">
          ...
          {mainTab === 'audits' && (
            <Select value={activeTab} onValueChange={(value) => setActiveTab(value as AuditView)}>
          ...
          {mainTab === 'actions' && (
            <div className="flex items-center gap-3">
              <Select value={actionsTab} onValueChange={setActionsTab}>
                ...
                <SelectContent>
                  <SelectItem value="all">All ({actionSource.length})</SelectItem>
                  <SelectItem value="overdue">Overdue ({actionSource.filter(a => a.status === 'overdue').length})</SelectItem>
                  <SelectItem value="open">Open ({actionSource.filter(a => a.status === 'open').length})</SelectItem>
                  <SelectItem value="in-progress">In Progress ({actionSource.filter(a => a.status === 'in-progress').length})</SelectItem>
                </SelectContent>
              </Select>
        ...
        {mainTab === 'audits' && (
          <div className="space-y-3 px-4">
            ...
            {/* Mobile Dropdown: Today | Upcoming | Completed */}
            <div className="md:hidden">
              <Select value={activeTab} onValueChange={(value) => setActiveTab(value as AuditView)}>
                <SelectTrigger className="w-full rounded-lg border-[#D1D1D1] bg-white">
                  <div className="flex items-center gap-2">
                    <Filter size={16} strokeWidth={1.5} className="text-[#605E5C]" />
                    <SelectValue />
                  </div>
            ...
            {selectedAudits.length > 0 && selectedAudits.map(renderAuditCard)}
          ...
          <div className="bg-white w-full rounded-t-2xl space-y-4 animate-in slide-in-from-bottom duration-300 pt-[24px] pr-[24px] pl-[24px] pb-[80px]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  <option value="all">All ({actionSource.length})</option>
                  <option value="overdue">Overdue ({actionSource.filter(a => a.status === 'overdue').length})</option>
                  <option value="open">Open ({actionSource.filter(a => a.status === 'open').length})</option>
                  <option value="in-progress">In Progress ({actionSource.filter(a => a.status === 'in-progress').length})</option>
                </select>
              </div>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          {selectedAction && (
            <div className="space-y-4 mt-[-15px] mr-[0px] mb-[0px] ml-[0px]">
              ...
              {(() => {
                const otherActions = actionSource.filter(a => a.auditId === selectedAction.auditId && a.id !== selectedAction.id);
                const ncCount = actionSource.filter(a => a.auditId === selectedAction.auditId).length;
                
                // Get audit response summary
                const auditSummary = auditSource.find(a => a.id === selectedAction.auditId)?.responseSummary || {
                  conforming: 8,
