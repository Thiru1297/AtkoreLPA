...
const questions: Question[] = [
  {
    ...
    comments: {
      required: true,
      label: 'Comments',
      placeholder: '1) NON-COMPLIANCE PHOTO AND FILL STOP TAG 2) IF NO ISSUES FOUND RECOGNIZE THE STACKER {TAKE PALLET LOCATION PICTURES}'
    },
    evidence: [
      { id: 'nc-photo', label: 'NC Photo', required: true }
    ]
  },
  {
    id: 2,
    ...
    comments: {
      required: true,
      label: 'Please input findings',
      placeholder: 'List tag numbers checked, identify any incorrect tags, and note corrective actions taken'
    },
    evidence: [
      { id: 'photo-1', label: 'Photo 1', required: false },
      { id: 'photo-2', label: 'Photo 2', required: false }
    ]
  },
  {
    id: 3,
    ...
    comments: {
      required: false,
      label: 'Comments on findings',
      placeholder: 'Specify workstation numbers checked, tag count at each location, and any non-compliance details'
    },
    evidence: [
      { id: 'workstation-1', label: 'Any workstation photo 1', required: false },
      { id: 'workstation-2', label: 'Any workstation photo 2', required: false }
    ]
  },
  {
    id: 4,
    ...
    comments: {
      required: true,
      label: 'Comments',
      placeholder: '1) NON-COMPLIANCE PHOTO AND FILL STOP TAG 2) IF NO ISSUES FOUND RECOGNIZE THE MATERIAL HANDLER {TAKE PALLET LOCATION PICTURES}'
    },
    evidence: [
      { id: 'nc-photo', label: 'NC Photo', required: true },
      { id: 'location-1', label: 'Pallet Location Photo 1', required: false },
      { id: 'location-2', label: 'Pallet Location Photo 2', required: false },
      { id: 'location-3', label: 'Pallet Location Photo 3', required: false }
    ]
  },
  {
    id: 5,
    ...
    comments: {
      required: false,
      label: 'Comments in shrink wrapping area',
      placeholder: 'Describe checklist completeness, identify missing information, and note compliance status'
    },
    evidence: [
      { id: 'checklist', label: 'Pallet stacking checklist photo', required: false }
    ]
  }
];
...
const groupQuestionsByDepartment = () => {
...
export function Questionnaire({ auditId, onNavigate, onBack, onExit }: QuestionnaireProps) {
  const departmentGroups = groupQuestionsByDepartment();
    ...
    setResponses({
      ...
      [questionId]: { 
        ...
        evidence: { 
          ...
          [evidenceId]: { file: `evidence-${evidenceId}-${Date.now()}.jpg`, comment: '' } 
        }
      }
    });
  };
  ...
  // Main accordion view
  const allAnswered = questions.every(q => responses[q.id]?.answer);
        ...
        <div className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-body-small text-[#6B7280]">
              {questions.filter(q => responses[q.id]?.answer).length} of {questions.length} answered
            </span>
          </div>
          <div className="h-2 bg-[#F3F2F1] rounded-full overflow-hidden">
            <div 
              className="h-full bg-[#4CAC48] transition-all duration-300"
              style={{ width: `${(questions.filter(q => responses[q.id]?.answer).length / questions.length) * 100}%` }}
            />
          </div>
        </div>
      </div>
